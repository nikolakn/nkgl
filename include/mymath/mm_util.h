...
#define MYMATH_MAKEPLANARSHADOW_FUNC(t) \
  ...
  template<typename t>
  inline impl::mat4i<t> perspective( const t& fovy, const t& aspect, const t& near1, const t& far )
  {
    ...
    t right = -left;
    impl::mat4i<t> r;
    r[0].x = (t)(2*near1)/( right - left );
    r[1].y = ((t)(2*near1))/ ( top - bottom );
    r[2].x = ( right + left ) / ( right - left );
    r[2].y = ( top + bottom ) / ( top - bottom );
    r[2].z = -( far + near1 ) / ( far - near1 );
    r[2].w = -( t )1;
    r[3].z = -( ( t )(2 * far * near1) ) / ( far - near1 );
    r[3].w = ( t )0;
    return r;
  }

  template< typename t >
  inline impl::mat4i<t> ortographic( const t& left, const t& right, const t& bottom, const t& top, const t& near1, const t& far )
  {
    impl::mat4i<t> r;
